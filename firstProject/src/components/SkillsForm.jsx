...
const SkillsForm = () => {
    ...
    // Fetch skills and interests for the logged-in user from Firebase
    const fetchSkillsAndInterests = async () => {
        const skillsQuery = query(collection(db, 'skills'), where('userId', '==', currentUser.uid));
        const skillsCollection = await getDocs(skillsQuery);
        const skillsList = [];
        const interestsList = [];
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await addDoc(collection(db, 'skills'), {
                ...
                userId: currentUser.uid // Store user ID with skill and interest
            });
